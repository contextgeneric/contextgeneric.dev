...
With the CGP function defined, let's define a minimal `Rectangle` context type and test calling `rectangle_area` on it:
...
To enable context-generic capabilities on a context, we first need to apply `#[derive(HasField)]` on `Rectangle` to generate generic field access implementations. After that, we can just call `rectangle_area` on it:
