...
**No ambiguity.** Scala's type-only resolution means two in-scope values of the same type create an ambiguity that requires explicit disambiguation. CGP resolves by *both name and type*: `#[implicit] width: f64` looks for specifically a field named `width` of type `f64`. Because Rust structs cannot have two fields with the same name, CGP implicit arguments are unambiguous by construction.
...
CGP v0.7.0 has now come with preliminary support for [agent skills](https://agentskills.io/home) for LLMs to use CGP. The [**CGP Skills**](/docs/ai-assisted-development/skills) document is specifically written to teach LLMs about CGP in a compact way.

If you would like to try out CGP, it is highly recommended that you include the CGP skill in your LLM prompts, so that you can ask it to clarify any CGP concept.
...
The `#[cgp_type]` macro no longer generate a type alias in the form `{Type}Of`. For example, given:
...
The type alias was originally generated to assist the use of abstract types within nested contexts. However, the new `#[use_type]` attribute provide significantly better ergonomics for such use cases. As a result, we no longer expect developers to use these type aliases to access the abstract types.

### Rename `ProvideType` to `TypeProvider`

The `HasType` CGP trait is used internally by `#[cgp_type]` when generating helper type providers. It was defined with a custom provider trait name `ProvideType` and a component name `TypeComponent`:
...
v0.7.0 updates the provider name to `TypeProvider`, and the component name to `TypeProviderComponent`:
...
This brings consistency to the naming convention we have for abstract type components that are generated by `#[cgp_type]`. For example, given the following trait:
...
The default provider name is `ScalarTypeProvider` and the component name is `ScalarTypeProviderComponent`.