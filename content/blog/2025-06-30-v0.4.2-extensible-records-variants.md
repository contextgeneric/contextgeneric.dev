...
Now that we have understood the value CGP offers on top of extensible data types, let's look at each of the field traits introduced by CGP to support extensible data types. We will start with the simplest trait of all: the `HasField` trait.

For the record, the `HasField` trait is already available since early versions of CGP. But we will have a short recap here to help bridge the knowledge gap toward more advanced constructs. The `HasField` trait is defined as follows:
...
The `HasField` trait is used for **read access** to a field in a struct. The trait is parameterized by a `Tag` type, which represents the field name at the type level. For structs with named fields, the `Tag` would be in the form `symbol!("first_name")`. While for structs with unnamed fields, the `Tag` would be in the form `Index<0>`.

The `Value` associated type represents the type of the corresponding field in a struct. The `get_field` method takes a reference to the struct and returns a reference to the field value. The `PhantomData<Tag>` value is passed as the second argument, to allow Rust to infer the `Tag` type when multiple `HasField` traits for the same struct is available in scope.

As an example, given the following struct:
...
The traits `HasField<symbol!("first_name"), Value = String>` and `HasField<symbol!("last_name"), Value = String>` are automatically derived for the `Person` struct.

The `HasField` trait helps us write datatype-generic code that needs read access to subsets of the fields of a struct. However, if we want enable the extensible builder pattern to construct subsets of a struct, we need to first setup some additional constructs.

## Partial Records

A key limitation of Rust structs is that when we construct a value, the values for all fields of the struct must be provided at the time of construction. To get around this limitation, CGP introduces the concept of **partial records** to represent a struct with some fields missing.

The way it works is as follows. Given the same example `Person` struct:
...
When the `BuildField` trait is derived, it would also generate a partial record for `Person`, called `PartialPerson`, as follows:
...
Compared to the original `Person` struct, the `PartialPerson` struct has two additional type parameters, `F0` and `F1`, which is required to implement the `MapType` trait, which is defined as follows:
...
That is, the `MapType` trait has a *generic associated type* (GAT) called `Mapped`, whose role is to map a type to *either* itself or its *absence*, depending on whether the value is present in the partial record.

For partial records, there are two kinds of `MapType` implementations. First, `IsPresent` maps a type back to itself to imply that the field is present:
...
Secondly `IsNothing` maps all types to the unit type `()` to indicate the *absence* of a field:
...
To see how the two type mappers are used, let's look at how a `PartialPerson` struct would look like if only the `first_name` field is present. The type would be instantiated as `PartialPerson<IsPresent, IsNothing>`, with `F0 = IsPresent` and `F1 = IsNothing`. So the struct becomes equivalent to:

```rust
pub struct PartialPerson {
    pub first_name: IsPresent::Mapped<String>,
    pub last_name: IsNothing::Mapped<String>,
}
```

which after `Mapped` is applied becomes:
...
So with that, we can construct a `PartialPerson` struct with only the `first_name` field present:
...
With the partial record struct defined, we can now define a `HasBuilder` trait for the struct, to initialize a partial record for the struct with *all fields absent*:
...
The `HasBuilder` trait has an associated type `Builder` that represents the initial partial record struct. The initial `Builder` must has all field empty, as the `builder()` method requires it to be built with no argument given.

For `Person`, the `HasBuilder` trait can be implemented as follows:
...
As we can see, the initial `Builder` type for `Person` is `PartialPerson` with all type mappper parameters set to `IsNothing`. To construct a value, we simply initialize all fields to the mapped type by `IsNothing`, which is always `()`.

### `BuildField` Trait

With the initial builder constructed. What we need next is to define the `BuildField` trait, so that the field values can be incrementally be built. The `BuildField` trait is defined as follows:
...
`BuildField` is parameterized by a `Tag` type that is used the same way as in `HasField`. Similarly, it has an associated type `Value` that represents the value type when the field is present.

In addition, the `BuildField` trait has an associated type `Output` that represents the type of the builder after the field is built. This is needed because as field values are inserted into the partial record, its generic parameters are updated, which would be treated as Rust as different types.

Finally, the trait has a `build_field` method that takes an *owned* `self` value, together with the constructed field `Value`, and returns the updated builder.

To understand how `BuildField` actually works, we will first look at how it is implemented for `PartialPerson`:
...
The first implementation `BuildField<symbol!("first_name")>` for `PartialPerson` is for the `first_name` field. The first generic parameter for `PartialPerson` is set to `IsNothing`, as we expect the field to be absent when the value is being constructed. For the second parameter, we parameterize it with a generic type `F1`, as our implementation would work *regardless* of whether the `last_name` field is present or not.

In the implementation body, we set `Value` to `String`, which is the field type of `first_name`. For the `Output` type, we set it to `PartialPerson` with the first parameter *updated* from `IsNothing` to `IsPresent`. The second parameter remains to be `F1`, meaning that if the `last_name` field is present, it will stay present, and vice versa.

Inside the `build_field` method, we construct a new `PartialPerson` with the `first_name` field updated to the `value` provided, and with `last_name` value moved from `self.last_name`.

Similary, there is also a `BuildField` implementation for the `last_name` field, which is defined as follows:
...
The main difference between the two implementations is that the first parameter of `PartialPerson` is now a generic `F0`, and the second parameter is `IsNothing`. Aside from that, the pattern for both implementations remain similar, aside from the orderings getting swapped.

With `BuildField` implemented, we can now use `HasBuilder` and `BuildField` to construct a `Person` value as follows:
...
It is also worth noting that with the way `BuildField` is implemented, the ordering of the fields being built does not matter. So we can also build `Person` with `last_name` first:
...
The earlier example shows how we can use the `builder` and `build_field` methods to idiomatically construct field values following the style of builder patterns. However, it is worth noticing that the result from the final call to `build_field` is actually `PartialPerson<IsPresent, IsPresent>`, *not* `Person`.

This is because for each `BuildField` implementation of `PartialPerson`, the implementation can only focus on a single field, with the presence status of all other fields masked by the generic parameters. This means that the `BuildField` implementation cannot "special case" on the case when the last remaining field is being built to return `Person` directly.

Although this makes the final construction a bit inconvenient, the generic implementation of `BuildField` is crucial to allow us to support the building of fields in any ordering, without needing to face the *combinatorial explosion* of explicitly implementing all possible combinations as separate implementations.

Instead, we introduce a `FinalizeBuild` trait, which is expected to be used after the build has completed to convert a `PartialPerson` back into a `Person`:
...
The `FinalizeBuild` trait has an `Output` type to represent the build result, and a `finalize_build` method that accepts an owned `self` value, and convert it into the `Output` type.

We would then implement `FinalizeBuild` for `PartialPerson` as follows:
...
As we can see, `FinalizeBuild` is only implemented for `PartialPerson` when all type parameters are `IsPresent`. This means that `PartialPerson` now has all fields present, and we can simply construct `Person` by assigning all field values from `self`.

With `FinalizeBuild` implemented, we can now add a final call to `finalize_build` to complete the build process for `Person`:
...
As we can see, the generated partial record struct together with the `HasBuilder`, `BuildField`, and `FinalizeBuild` traits serves as a simple but strong foundation for CGP to support the **extensible records** features that we have covered in earlier sections.

The partial record struct and the relevant traits are all generated from the `#[derive(BuildField)]` macro. We derive them within the same macro, both as a convenience, and also for consistency to prevent compilation failure if the user failed to specify derive for all the required traits and struct.

## `BuildFrom` Implementation
