...
We define `BuildWithHandlers` as the primitive builder dispatcher for implementing higher-level dispatchers such as `BuildAndMergeOutputs`. To understand how the higher level dispatchers work, we will first look at some example direct use of `BuildWithHandlers`.

Suppose that we have a `Computer` provider to build a `Person`, and a `Computer` provider to build the `u64` *value* of `employee_id`, we can use `BuildWithHandlers` to compose the two providers together to build `Employee`.

We would first implement two dummy providers to build `Person` and `u64`. When we want to implement providers that can work with any `Context`, `Code`, and `Input`, we can use `#[cgp_producer]` as a short hand to implement them as a *function*:
...
Behind the scene, `#[cgp_producer]` generates the corresponding providers with camel-cased names (`BuildPerson` and `BuildEmployeeId`), and implement traits like `Computer` by calling the given functions.

Helper macros like `#[cgp_producer]` helps us significantly reduce the boilerplate code required to turn *pure functions* into handler providers, especially when the generic `Context` and `Code` parameters are ignored.

With the defined providers, we can then build an `Employee` using `BuildWithHandlers` as follows:
...
We first pass `Employee` as the first argument to `BuildWithHandlers`, indicating that we want the build result to be `Employee`. We then pass a list of two builders as the second argument. First, `BuildAndMerge<BuildPerson>` wraps around `BuildPerson` by first calling `BuildPerson`, and then use `BuildFrom` to merge the result into `PartialEmployee`. Second, `BuildAndSetField` wraps around `BuildEmployeeId` by first calling `BuildEmployeeId`, and then use `BuildField` to set the field `employee_id` to the result returned.

We call `compute` on the specialized `BuildWithHandlers` with dummy `Context`, `Code`, and `Input` with all being just `()`. In practice, especially for more complex applications, the `Context` and `Code` parameters may be instantiated with proper types.

This example demonstrates that there are more than one ways we can compose builders using `BuildWithHandlers`. While earlier examples define builder providers that return record subsets that can be merged, it is also possible to compose builder providers that return unwrapped values of individual fields, and insert the value into a selected field.

### `BuildAndMerge`

The implementation behind `BuildAndMerge` is pretty simple, and is defined as follows:
...
`BuildAndMerge` wraps around an inner provider, such as `BuildPerson`, and accepts a generic `Builder` as input, which would be the partial record for the end result struct.

It expects the inner provider to implement `Computer` with a *borrowed* `&Builder` as the input. This input allows the inner provider to read any field that is already set in the partial record, so that intermediary build results can be used to build other field values. For the case of `BuildPerson`, since the input is ignored, the constraint is always satisfied.

After the inner provider returns the result, `BuildAndMerge` expects the `Builder` partial record to implement `BuildFrom` for the returned value, which would then be used to merge the result fields into `Builder`.

### `BuildAndSetField`

The implementation of `BuildAndSetField` is pretty similar, and is defined as follows:
...
Compared to `BuildAndMerge`, `BuildAndSetField` instead requires `Builder` to implement `BuildField` for the provided `Tag`. It takes the value returned by the inner `Provider`, and then set it as the value for the given field.
