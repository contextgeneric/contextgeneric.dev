...
This mismatch becomes especially noticeable when using compact formats like [`postcard`](https://docs.rs/postcard) or [`bincode`](https://docs.rs/bincode), which support only a small subset of types compared to JSON. These libraries allow any type implementing `Deserialize`, but compatibility is only verified at runtime—leaving users to discover format mismatches through error messages rather than compile-time guarantees.

In short, the traditional visitor pattern tends to be either too **restrictive** (by enforcing a closed set of operations) or too **permissive** (by relying on runtime validation to reject unsupported operations). What’s needed is a more flexible, composable alternative—one that allows both sides (visitor and visitee) to express **precise, compile-time requirements**.
...
Suppose we want to implement an interpreter for a toy arithmetic language in Rust. This language might support basic expressions like `1 + (2 * 3)`. A typical way to represent such a language is with an enum like this:
...
Here, `Expr` represents arithmetic expressions. The `Plus` and `Times` variants contain boxed sub-expressions, and `Literal` holds an integer value. The use of `Box` is necessary due to Rust’s size constraints for recursive data structures.
...
Worse, this coupling is not easy to break. The recursive nature of `Expr` — where variants like `Plus` contain other `Expr` values — means even modular helper functions (e.g., `eval_plus`) must still operate on `Expr`, perpetuating the tight dependency.
...
This implementation defines `EvalAdd` as a `Computer` with `Plus<Expr>` as input. It works generically over any `Context`, `Code`, and `Expr` type, as long as the context knows how to compute `Expr` and the resulting output type supports the `Add` trait. In other words, the context must be able to evaluate each operand, and the results must be addable.
...
With our evaluation logic defined for individual expression types like `Plus`, `Times`, and `Literal`, the next step is to bring everything together into a fully functional evaluator. To do this, we first define a concrete expression type named `Expr`:
...
* `Expr` is mapped to `DispatchEval`, which acts as a dynamic router that dispatches to one of the subtypes based on the variant.
* `Plus<Expr>` is evaluated using `EvalAdd`.
* `Times<Expr>` is evaluated using `EvalMultiply`.
* `Literal<u64>` is handled by `EvalLiteral`.

Because `UseInputDelegate` operates at the type level, the entire dispatch process is fully type-safe and resolved at compile time. There is no need for `match` statements, no runtime type checks, and no boilerplate glue code. The system simply composes itself from the parts we’ve defined.

## Dispatching Eval

With the components for evaluating individual sub-expressions in place, we now turn our attention to the final piece of the puzzle: evaluating the main `Expr` enum itself. To accomplish this, we delegate the `Expr` type to a special provider named `DispatchEval`, which is defined alongside the `Interpreter` context like so:
...
This demonstrates the basic structure. But the real point is this: **converting from `Expr` to `LispExpr` is itself another instance of the expression problem**, just like evaluation. In fact, it's even more subtle—this is a **"double expression problem"**: we want to decouple our logic from both the *source expression type* (`Expr`) and the *target type* (`LispExpr`).

So how do we solve it modularly?

### The `ComputerRef` Component

To implement this transformation, we’ll use a slightly different CGP trait: `ComputerRef`.
...
With our expression types and Lisp target representation in place, we can now implement a CGP provider that transforms a `Plus` expression into its corresponding Lisp representation. This transformation is handled through an implementation of the `ComputerRef` trait, which allows us to work with references rather than consuming the input. Here's what the provider looks like:
...
This implementation takes a `Plus<Expr>` as input and returns a `LispExpr` as output. The transformation is recursive: each subexpression is converted by delegating to the same `ComputerRef` trait for `Expr`. The resulting `LispExpr` values are then combined into a list, with the `"+"` operator represented as an identifier at the head.

Notice that the provider is generic over both the context and the code. It requires that the context knows how to evaluate an `Expr` into a `LispExpr`, and that it defines what `LispExpr` actually is. This is done via a trait called `HasLispExprType`:
...
By relying on this trait, we avoid hardcoding the `LispExpr` type directly into the provider. Instead, the actual type can be supplied later when we wire everything together. This decoupling is one of CGP’s strengths—it allows the transformation logic to remain generic and reusable, regardless of how the final expression type is defined.
...
This approach gives us the best of both worlds: generic, reusable logic with precise control over what each component is responsible for. It is this composability and type safety that makes CGP especially compelling for expression transformations like this one.

### Literal To Lisp Implementation

The implementation of `TimesToLisp` is similar to `PlusToLisp`, other than the fact that we need to construct a Lisp identifier `*` instead of `+`. So we will skip the implementation details and move on to the implementation of `LiteralToLisp`:
...
For the implementation of `LiteralToLisp`, our local `LispSubExpr` is parameterized by the literal type `T`, and contains only the `Literal` variant. Aside from that, we also require `T` to implement `Clone`, so that we can clone the literal value from the original expression.

It is worth noticing here that the `Literal` type we used for `LispExpr` is the same `Literal` type we used for the arithmetic `Expr`. This demonstrates the flexibility of defining separate types for each variant in an expression type. This allows us to reuse not just the type, but potentially also the *providers* of one language implementation in a completely different language. For example, we can reuse the `EvalLiteral` provider we defined earlier also for evaluating the literal variant in Lisp expressions.

### Wiring To-Lisp Handlers

With our to-Lisp providers in place, the final step is to wire them into the interpreter cotnext. This is where we connect all the pieces together in the `delegate_components!` macro by updating the `InterpreterComponents` definition to include the Lisp transformation logic:
...
In this configuration, we begin by wiring `LispExprTypeProviderComponent` to use `LispExpr` as the concrete type for the abstract `LispExpr` we’ve been referencing in our providers. This is handled by the `UseType` provider, which instantiates the abstract type used in our providers with the actual enum definition.

The `ComputerComponent` remains unchanged from our evaluation setup. It delegates expression evaluation to the appropriate providers based on input types such as `Plus<Expr>` and `Literal<u64>`.

The new addition here is the `ComputerRefComponent`, which is responsible for reference-based computation — specifically, transforming expressions into their Lisp form without consuming them. As with the evaluation component, we use `UseInputDelegate` to dispatch inputs to the appropriate transformation providers. This includes `PlusToLisp`, `TimesToLisp`, and `LiteralToLisp`.

For the top-level `Expr` enum, we delegate to `DispatchToLisp`, which is defined similarly to `DispatchEval`, but targets reference-based dispatching:
...
The key difference here is the use of `MatchWithValueHandlersRef`, a CGP-provided dispatcher designed for working with borrowed inputs. It operates similarly to `MatchWithValueHandlers`, but invokes `ComputerRef` instead of `Computer`, and ensures that values are passed by reference.

What’s powerful about this approach is that it’s entirely type-driven and compile-time safe. Because the dispatcher is implemented generically — rather than as a macro — it can take full advantage of Rust’s type system to catch errors early. Lifetime mismatches, invalid borrow scenarios, or incorrect trait bounds are all handled by the compiler. If this logic had been implemented with traditional macros, these kinds of mistakes could easily slip through, only surfacing at macro expansion time or during later stages of debugging.

Moreover, `MatchWithValueHandlersRef` is just one of several dispatchers available in CGP. Each of them shares a common, composable foundation, and they can be mixed, extended, or even replaced with custom implementations tailored to your application. This flexibility is a direct result of CGP’s generic programming model, which allows you to build sophisticated behavior without losing control over types or falling back on fragile macro magic.

By wiring both evaluation and Lisp transformation into the same interpreter context, we now have a fully working setup where a single `Expr` type can be both evaluated to a numeric result and transformed into a Lisp-like syntax tree. This separation of logic into modular, type-safe components is one of CGP’s greatest strengths — it gives you a scalable foundation for building interpreters, compilers, or any structured transformation pipeline in Rust.

### Binary Operator Provider

If we look at the implementation of `PlusToLisp` and `TimesToLisp`, you can notice that the implementation for both are almost identical, other than the operator symbol used and the input types.

We can extract the common logic, and implement a common provider that works for *both* `Plus` and `Times` as follows:
...
```
The `BinaryOpToLisp` provider is parameterized by an `Operator` type, which represents either `"+"` or `"*"` as type-level strings. The `Operator` is required to implement `Default` and `Display`, so that we can format the type as a string.

The provider works with any `MathSubExpr`, such as `Plus` or `Times`, as input, given that the sub-expression implements `BinarySubExpression` which is defined as follows:
...
We use `#[cgp_auto_getter]` to automatically implement the trait for any type that contains a `left` and ` right` field of type `Box<Expr>`. We then bind the generic `Expr` parameter with the abstract `MathExpr` type, which is defined by `HasMathExprType`:
...
Similar to `HasLispExprType`, we introduce `HasMathExprType` to define `MathExpr` as an abstract type instead of as a generic parameter. This helps us uniquely bind the type parameter at `BinarySubExpression` and avoid ambiguous implementation.

The method body of `BinaryOpToLisp` is almost identical to the original implementation of `PlusToLisp` and `TimesToLisp`. The main difference is that we use `Operator::default().to_string()` to construct the `LispSubExpr::Ident` variant, instead of a hard-coded operator.

With `BinaryOpToLisp` defined, we can now replace our ad hoc implementation of `PlusToLisp` and `TimesToLisp`, and wire them up to both use `BinaryOpToLisp`:
...
As we can see, with the use of `UseInputDelegate` to dispatch computation based on input, and the automated dispatch of variants as inputs to inner computation by `MatchWithValueHandlersRef`, we have unlocked much higher expressiveness, which allows us to build and use reusable components like `BinaryOpToLisp` much more easily.
