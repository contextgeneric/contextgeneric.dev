...
This macro allows us to delegate the implementation of various components of the builder context. First, we configure error handling by using the `cgp-anyhow-error` library. The `UseAnyhowError` component specifies that our abstract `Error` type will be instantiated to `anyhow::Error`, and the `RaiseAnyhowError` component allows conversion from error types implementing `core::error::Error`, like `sqlx::Error` and `reqwest::Error`, into `anyhow::Error`.

For the `HandlerComponent`, we use a special **builder dispatcher** called `BuildAndMergeOutputs`. This dispatcher enables us to construct the full `App` type by combining multiple builder providers. The `App` type is specified as the target of the build, and we pass a type-level list of the builder providers to be used, wrapped in `Product!`. In this example, we include `BuildSqliteClient`, `BuildHttpClient`, and `BuildOpenAiClient`, which we implemented earlier.
...
In this example, we initialize `FullAppBuilder` by filling in the required configuration values. We then call `builder.handle()` to construct the `App`. The `handle` method requires two arguments: a `Code` type and an `Input` value. However, because neither of these are constrained in any way in our example, we can simply pass any type we want, i.e. the unit type `()` for both. This simplifies to the equivalent of calling `builder.handle()` with no argument in practice.

From this example, we can see how CGP enables new builder contexts to be defined easily, by wiring up multiple builder providers that do not have access to the concrete target type to be built.

Instead of defining custom constructor functions that accept many arguments, we instead define a builder struct with the arguments becoming the struct fields. Instead of constructing each context field manually, we use `delegate_components` to wire up the relevant builder providers to construct the field values for us.

With the modular builders, our code not only becomes significantly easier to extend, but also much simpler to read and maintain.

## More Builder Examples

At this point, some readers may still not be convinced of the value of modularity provided by CGP builders. Since we have only defined _one_ application context and _one_ builder context, it would not seem obvious why can't the application be constructed with the simpler `new` constructor that we have defined at the beginning.

To show the true power of modular builders, it would be necessary for us to elaborate on the building of _many_ similar but slightly different contexts. However, at the risk of boring more **advanced** readers who already find such benefits to be obvious, we advice these readers to [**skip to the next section**](#modular-visitor-example) to read about more interesting content.

### Default Builder

Recall that earlier we have defined default builders such as `BuildDefaultSqliteClient`, which support construction of `App` using default values. We can combine these default builder providers together to form a default builder for `App` as follows:
...
For this `DefaultAppBuilder` context, the only value that is required by the builder is `db_path` for the file path to the SQLite database. With this builder, we can much easily construct `App` in places such as unit tests.

### Postgres App

Now suppose that instead of using SQLite, we want the enterprise version of our application to use Postgres as the database. We would now define a different `App` context that contains `PgPool` as the database pool as follows:
...
Given that we can still reuse all existing HTTP and ChatGPT builders that we have defined earlier, all we need to do next is to implement a new builder provider for Postgres, such as follows:
...
The Postgres client builder is similar to the SQLite client builder, except that it is expecting a `postgres_url` field from the builder context.

Next, we will define a new `AppBuilder` that contains `postgres_url` instead of `db_path`:
...
Inside the `delegate_components!` wiring, we choose to use `BuildPostgresClient` instead of `BuildSqliteClient`. But we are able to continue to reuse `BuildHttpClient` and `BuildOpenAiClient` to build the HTTP and ChatGPT clients.

As we can see through this example, CGP serves as a compelling alternative to the use of **feature flags** to tweak application behavior. With feature flags, we would only able to build _either_ an SQLite version _or_ a Postgres version of the application. But with CGP, _both_ versions of the `App` can _co-exist_ inside different modules or different crates, and included in the same compilation step.

This significantly simplifies how the application can be tested, as we don't need to worry about missing to test a version of the application that is made of specific feature flags, especially when there are many other feature flags present.

### Anthropic App

Similar to switching SQLite with Postgres, we can also opt to use a different AI model for our application, such as using Claude instead of ChatGPT. With CGP, we can now define a separate `AnthropicApp` that contains an Anthropic client and agent as follows:
...
We can then implement a builder provider to build the Claude client for us:
...
Once that is done, we just need to define a new builder context that contains the Anthropic API key, and wired with `BuildDefaultAnthropicClient`:
...
As we can see, the same pattern and steps can be repeated over and over to customize the application in any way we want. In fact, the pattern is probably obvious enough that given the correct prompts and documentation, it should be easy for vibe coding tools like Claude Code to do the full customization for us.

### Anthropic and ChatGPT Builder

It is pretty awesome that CGP allows us to switch our application from ChatGPT to Claude easily. But what's even awesome is why not include **both** AI agents together for our app?

This might perhaps be useful for some folks who believe that using multiple AIs can boost the intelligence of their application. But the main point we want to show here is that CGP is not only offering us a **choice** between multiple providers, but also a way for us to **combine** the providers to ues them all at once.

We would first define a `AnthropicAndChatGptApp` context, with both Claude and ChatGPT clients:
...
We then define an `AnthropicAndChatGptAppBuilder` that contains the builder parameters for both Claude and ChatGPT:
...
Inside the component wiring, we now include **both** `BuildDefaultAnthropicClient` and `BuildOpenAiClient` in our list of builder providers:
...
With just few lines of code, we are now able to build an AI application that is twice as intelligent (/s).

Furthermore, notice that the `llm_preamble` field is reused by both `BuildDefaultAnthropicClient` and `BuildOpenAiClient`. This shows the flexibility of CGP to implicitly propagate the same build parameter to multiple builder providers, without us having to pass them manually.

### Multi-Context Builder

Moreover, if we look at the new `AnthropicAndChatGptAppBuilder` that we have defined, it also contains all necessary fields to build the Claude-only and ChatGPT-only apps that we have defined earlier. In fact, we can modify the wirings to enable the building of **all** three versions of the app contexts with the same builder.

We can do this by making use of the `Code` parameter, to build [DSL-like syntaxes similar to Hypershell](/blog/hypershell-release/#abstract-syntax):
...
We can then make use of [`UseDelegate` pattern](/blog/hypershell-release/#generic-dispatcher) to _dispatch_ the `Handler` implementation to different builder pipelines, depending on the `Code` that we have defined:
...
Now when we want to build our application contexts, we just need to customize the `Code` parameter by passing them through `PhantomData`, such as follows:
...
pub async fn main() -> Result<(), Error> {
    ...
    let chat_gpt_app: ChatGptApp = builder.handle(PhantomData::<BuildChatGptApp>, ()).await?;

    let anthropic_app: AnthropicApp = builder.handle(PhantomData::<BuildAnthropicApp>, ()).await?;

    let anthropic_and_chat_gpt_app: AnthropicAndChatGptApp = builder
...
This example shows that the DSL capabilities in CGP is not just useful for building full-blown DSLs like [Hypershell](/blog/hypershell-release/) - They are are also useful for _labeling_ different program behaviors that arise from different combination of providers.

In fact, one way to think of this is that we are still effectively building a mini-DSL here, except that our mini-DSL is only consist of simple statements with no additional language constructs. This also opens door to possible extension of the builder pattern in the future by adding new abstract syntaxes using the techniques introduced by Hypershell.

# Modular Visitor Example