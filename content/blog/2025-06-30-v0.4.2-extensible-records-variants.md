...
To complete our evaluator, we need a way to connect the top-level `Expr` enum to the correct sub-expression evaluators. This is where the `DispatchEval` provider comes into play. It acts as a central dispatcher that knows how to interpret the `Expr` enum by delegating to the appropriate computation logic based on the variant.

Here’s how we implement `DispatchEval` using CGP:
...
This implementation is declared specifically for the `Interpreter` context, with the input type set to `Expr`. The output type is fixed as `u64`, aligning with our earlier decision to use `u64` for literals.

At the heart of this implementation is a call to `MatchWithValueHandlers`, a powerful and generic **visitor dispatcher** provided by CGP. It statically inspects the `Expr` variant and delegates the computation to the correct provider — `EvalAdd`, `EvalMultiply`, or `EvalLiteral` — based on the mappings we established in `EvalComponents`.

From a structural perspective, this implementation acts as a **wrapper** around the visitor dispatcher. This might seem redundant at first glance, but it's actually a clever workaround for a tricky compiler limitation in Rust.

### Why the Wrapper Is Necessary

Due to how Rust handles trait resolution, trying to directly implement `Computer` for `MatchWithValueHandlers` would lead to a cyclic dependency issue. Here's why: `MatchWithValueHandlers` needs to dispatch to each variant's provider — like `EvalAdd` — but those providers themselves require access to `Computer` implementations, which would then refer back to `MatchWithValueHandlers`. This creates a recursion loop that Rust is unable to resolve on its own.

By wrapping `MatchWithValueHandlers` in a context-specific provider like `DispatchEval`, we effectively break the cycle. Rust marks the trait as implemented at the wrapper level, and only afterward evaluates the logic within. This pattern lets the compiler proceed safely and compile successfully.

### What Actually Happens Behind `MatchWithValueHandlers`

To give you a better idea of what `MatchWithValueHandlers` is doing, here’s an approximation of the logic it performs internally:
...
The idea is straightforward: based on the variant of `Expr`, we forward the computation to the relevant provider. The difference is that `MatchWithValueHandlers` does this without hardcoding any enum — it works *generically*. This means you don’t have to write or maintain `match` statements yourself, and you avoid relying on code generation or macro expansion to automate them.

While this might not seem like a huge win for our small `Expr` enum with only three variants, the benefits become clear at scale. Consider something like `syn::Expr` from the Rust parser ecosystem, which has over 40 variants. Manually dispatching on that enum would be verbose and tedious. With CGP’s dispatcher, the entire structure becomes composable, modular, and significantly easier to maintain.

What makes CGP’s dispatching approach particularly compelling is that it avoids the pitfalls of procedural macros. Instead of relying on code generation, the dispatcher is just a normal Rust implementation — generic, type-safe, and fully verifiable by the compiler. That means it can be reused for *any* enum that satisfies the right constraints, regardless of where or how the enum is defined.

In essence, `MatchWithValueHandlers` is a generic, reusable engine for type-safe variant dispatching. It gives you all the flexibility of the visitor pattern, without the boilerplate or macro overhead.

## To Lisp Expression

We have now implemented a minimal evaluator for our toy language. But as with many other languages, there are often more than one ways we want to perform computation on the `Expr` type. For example, we may want to convert our expression into a _string_, or more generally, a _token stream_.

Although we could implement a modular `to_string` functionality with CGP, some reader might consider the problem to be too "trivial" to be solved using CGP. (spoiler: it's not) However, for the sake of making the example slightly more interesting and convincing, we will instead try and convert our arithmetic expression into a _Lisp expression_.

The reason we choose the conversion to Lisp expression as an example is to mirror the conversion of Rust expressions from a [`syn::Expr`](https://docs.rs/syn/latest/syn/enum.Expr.html) into a [`TokenStream`](https://docs.rs/proc-macro2/latest/proc_macro2/struct.TokenStream.html), i.e. a list of [`TokenTree`](https://docs.rs/proc-macro2/latest/proc_macro2/enum.TokenTree.html). However, since the `TokenTree` type is relatively complex, we have opt to use a simplified Lisp expression type to emulate the same operation for our example.

### A Quick Tour of Lisp

As a recap, a Lisp expression, i.e. [**S-expression**](https://en.wikipedia.org/wiki/S-expression), is made of _linked lists_ and _atoms_. For our example, we would want to encode our arithmetic expression from something like `1 + (2 * 3)` into `(+ 1 (* 2 3))`. To see how the converted value is represented, let's see how a basic Lisp-like expression type could be defined:
...
Compared to our arithmetic `Expr`, the `LispExpr` type is more general, and is designed to accomodate other Lisp expressions that may not be valid arithmetic expression. For simplicity, we define `LispExpr` to be made of either a `Vec` list of `LispExpr`, or a literal `u64` value, or an identifier made of string. We chose to use `Vec` instead of the classic linked list to make it easier for readers who are unfamiliar to Lisp to follow.

Using `LispExpr`, an expression like `(+ 1 (* 2 3))` could be manually constructed like follows:
...
Just as with other programming languages, the actual expression type for Lisp may also contain many other variants, and working with it also subjects to the same *expression problem* that we had for implementing arithmetic expression. In other words, the task for converting an arithmetic expression into Lisp expression is a **doubly expression problem**, as we need to decouple our implementation from not just the arithmetic `Expr`, but also the new `LispExpr`.

### The `ComputerRef` Component

Before we start, we first switch to use the `ComputerRef` trait provided by CGP to implement out to-Lisp converters:
...
The `ComputerRef` component has a `compute_ref` method that accepts a _reference_ to the `Input` parameter. This is in contrast to the `Computer` component, which has a method that accepts an _owned_ value for `Input`.

We opt to use `ComputerRef` for the to-Lisp conversion, so that the original expression value is *borrowed* instead of *moved*, so that we can reuse it later to perform operations like `eval`.

There are other alternatives to using `ComputerRef`, such as *cloning* the expression value, or using [*higher-ranked trait bounds*](https://doc.rust-lang.org/nomicon/hrtb.html) inside `Computer` to pass a *reference* as the `Input` type. However, we use `ComputerRef` here to *demonstrate* the options available, and provide you with an easy way to pass the `Input` by reference.

In practice, CGP provides conversion providers to *promote* a `ComputerRef` provider to also implement `Computer`. So we can think of the two traits being closely related and interchangeable in many cases. As a result, we can freely implement our providers using either `Computer` or `ComputerRef`, depending on whether we need to borrow or own the input.
