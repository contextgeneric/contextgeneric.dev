...
Now that we have covered the ground work for extensible records, we will next explore how CGP implements extensible variants. Contrary to what you might think, the way extensible variants are implemented is in fact very similar to the approaches we used for extensible records - we are just implementing things in the "opposite" direction.

The fact that extensible records and variants share similar implementations that complement each other has their roots traced back to the fundamentals of [**category theory**](https://bartoszmilewski.com/2015/01/07/products-and-coproducts/), where records are called **products** and variants are called **sums** or **coproducts** - indicating the deep duality relationship between the two concepts. Our implementation in CGP merely follows along this research direction, and explore the deeper duality relationship through further research in [extensible data types](https://dl.acm.org/doi/10.1145/3290325).

With that in mind, let's take a look at the CGP constructs that enables extensible variants. Throughout the exploration, we encourage you to try and observe and similarity and differences between these constructs and their counterparts for extensible records.

## `FromVariant` Trait

While extensible records have the `HasField` trait for us to read a field from a struct, extensible variants have the `FromVariant` trait for us to **build** an enum from a variant value. The trait is defined as follows:
...
Similar to `HasField`, `FromVariant` has a `Tag` parameter for the variant name, and an associated `Value` for the variant type. Instead of returning `Value`, the `from_variant` method accepts a `Value` and returns `Self`.

### Example `FromVariant` Derivation

As an example, given the following enum:
...
The following `FromVariant` implementations are generated:
...
### Limitations on Supported Enums

There is a limitation on the enums supported by the macros for extensible variants, such as `FromVariant`, in that the enum must be in a form called **sums of products**. That is, each variant in the enum must have exactly one unnamed field.

In other words, enums like the following are not supported:
...
The reason these forms of variants are not supported is so that CGP can provide more ergonomic APIs for working with extensible variants. If we were to support these other forms of variants, we would not be able to represent the standalone variant fields as types that remain ergonomic.

On the other hand, by restricting the enum variants to contain exactly one unnamed field, it becomes easy to determine what the types such as `FromVariant::Value` should be. As a result, we recommend that developers who want to use extensible variants with non-standard variants to wrap the fields into a separate struct type.
