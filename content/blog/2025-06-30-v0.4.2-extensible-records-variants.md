...
Very often, there is no single right way to construct a context. For example, we may want to keep both versions of `new` in the earlier example, so that unit tests can use the simpler constructor with default values, while the full application will use the fully configurable constructor. It is also often the case that certain parts of an application would benefit from a mix of default values and configurable values.

In today's Rust, the best practices to work around this problem is to use the [_builder pattern_](https://rust-unofficial.github.io/patterns/patterns/creational/builder.html), which is to define a separate builder struct that contains the values to build a context, as well as mutable methods to update these values.

## Challenges for Modular Builders

However, a key limitation with the builder pattern today is that the builder itself is _non-extensible_, and is _tightly coupled_ with the context type that it is building. In other words, if we have a similar but different context defined, we would have to duplicate the whole builder implementation for the new context. Furthermore, if we want to _extend_ how an existing builder constructs a context, it may be challenging to do so unless we directly make changes on the builder's source code.

A key reason for this challenge is that the construction of a struct requires direct access to its concrete type. As a result, a builder necessarily needs to know the concrete struct that it is building to build the struct.

Furthermore, the builder needs to keep track of all intermediary values that will be used for building the struct. But if we want to introduce new builder methods that require additional intermediary values, it would require direct modification of the builder struct itself.

## Modular Builder with CGP

Before the current release, the earlier versions of CGP also faced similar limitations. When writing context-generic code, we want to be able to write generic constructor code that can work with any compatible context type. But since the construction of a struct requires all field values to be presence all at once, we could not easily implement context-generic constructors without knowing the concrete type that it needs to construct.

But with this release, this limitation has been fully lifted, and we are now able to compose multiple context-generic builders, and construct a generic context that is fully decoupled from the builders.

We will now walk through how we would write the same example constructors that we have covered earlier, using the new CGP builder pattern that we have developed.