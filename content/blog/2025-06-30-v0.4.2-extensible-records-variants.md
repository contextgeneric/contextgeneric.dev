...
# Extensible Visitor Example

We have previously seen how the extensible builder pattern enables modular construction of contexts structs in CGP. Next, we will now look at how similar techniques can be applied to context **enums**, making it possible to break down the destructuring and pattern matching of each variant to be handled by a flexible group of handlers.

In Rust and many OOP languages, this pattern is also commonly known as the [**visitor pattern**](https://rust-unofficial.github.io/patterns/patterns/behavioural/visitor.html). Compared to mainstream OOP languages that lack of the equivalent to enums, there are less needs for using the visitor pattern in Rust. This is because when the concrete enum type is known, it is much easier to use the `match` expression to pattern match on each variant to handle the different cases.

Nevertheless, the visitor pattern is still used in many Rust crates today, including [`serde`](https://docs.rs/serde/latest/serde/de/trait.Visitor.html) and [`syn`](https://docs.rs/syn/latest/syn/visit/trait.Visit.html). They are especially useful when we need to write code without knowing the concrete enum type. For instance, the `serde` deserializer uses a visitor provided by a concrete type to navigate the internal structures of serialization payloads such as JSON and TOML, so that the deserializer is not tightly coupled with a specific serialization format.

## Problems with the Visitor Pattern

However, a key limitation of the visitor pattern is that all possible variants must be specified up front in the visitor interface, and cannot be extended later on. For example, the visitor trait in [`serde`](https://docs.rs/serde/latest/serde/de/trait.Visitor.html) only supports integers up to 128-bit precision. So suppose that a struct contains a [`U256`](https://docs.rs/primitive-types/latest/primitive_types/struct.U256.html) value, it cannot add a visitor method to extract a native 256-bit field from a serialization payload. Similarly, if someone wants to design a new serialization format to support a new field type like 256-bit integers, they would have issues integrating with the `serde` crate to support the use of this new field type during deserialization.

In order maximize interoperability, the `serde` crate tries to come up with up to 26 visitor methods up front in its `Visitor` trait. However, this not only bloats the interface, but also creates the opposite problem - when a serialization format does not support a specific visitor method, there is no way for it to reject the method other than to return an error during runtime. In other words, a user cannot know during compile time that a Rust type is truly compatible with a serialization format, even if the type implements `Serialize` or `Deserialize` successfully.

This issue frequently comes up when using `serde` with serialization formats such as [`postcard`](https://docs.rs/postcard) and [`bincode`](https://docs.rs/bincode) especially when the formats only support a subset of common formats such as JSON. Although these libraries provide methods that accept any type that implement `Serialize` or `Deserialize`, the user could only find out that a type is not actually supported when the method returns errors during runtime.

In summary, we have seen an example of how `serde` attempts to use the visitor pattern to provide an interoperability layer between Rust types and serialization formats. However, due to the non-extensibility of the visitor pattern, this result in the layer becoming either too _restrictive_ or too _permissive_ for either side of the implementations.

What we need is a more _extensible_ version of the visitor pattern, where both sides can specify the _precise_ requirements they need from the other side. And any unsatisfied requirement, should result in errors raised during _compile-time_ instead of run-time. This is basically what the **extensible visitor pattern** in CGP offers to solve.