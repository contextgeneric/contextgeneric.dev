...
With the `BuildFrom` trait implemented, we can now look at how CGP implements generalized **builder dispatchers** to generically build up struct fields in various ways.

In the earlier examples, we used `BuildAndMergeOutputs` to combine the outputs from builder  providers like `BuildSqliteClient`, `BuildHttpClient`, `BuildOpenAiClient`. Behind the scene, `BuildAndMergeOutputs` is built using a lower level dispatcher called `BuildWithHandlers`, which is implemented as follows:
...
`BuildWithHandlers` is parameterized by an `Output` type, which represents the concrete struct that we want to build, and a `Handlers` type in the form of a type-level list of handlers that can be used together to build the `Output` struct.

The provider implements `Computer` that works with any `Context`, `Code`, and `Input`, with the `Input` being ignored. It first require `Output` to implement `HasBuilder`, so that it can use `Output::builder()` to construct an initially empty partial record for `Output`.

Using this partial record, the provider then delegates the implementation to `PipeHandlers<Handlers>`, which is expected to implement `Computer` with `Output::Builder` as the input type.

Finally, the provider expects that the `Output` returned from `PipeHandlers` implements `FinalizeBuild`, with the finalized output being the `Output` type. This means that it expects `PipeHandlers` to take in an empty partial record, fill in the field values of that partial record using handlers, and return a fully populated partial record that can be finalized back into the main `Output` type.

Yes, and in case if you are wondering, this is the *same* `PipeHandlers` that we have previously used in [Hypershell](/blog/hypershell-release) to implement the pipelines for shell-like commands! In other words, `BuildWithHandlers` works by initializing an empty partial record, pipe it through a list of handlers, and then finalize the partial record into the main `Output` type.

Hopefully this also shows you how generalized and useful are the constructs in CGP, with them being usable in a wide range of scenarios. When we introduced constructs like `Pipe` and `Handler` in Hypershell, we were not just implementing them to pipe shell commands, but also _any_ kind of [functional composition](https://en.wikipedia.org/wiki/Function_composition_(computer_science)) that is commonly used in functional programming. So I hope that you are also convinced that Hypershell pipes are not just regular shell pipes, and become more open minded to think about new use cases for Hypershell beyond just shell programming.
