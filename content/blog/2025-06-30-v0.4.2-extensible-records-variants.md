...
Now that we have implemented a basic language that supports addition and multiplication, let's try and slightly extend the language with additional features. For this demonstration, we will try and add support for _subtraction_ and _negation_ as an _extension_ to the base language.

Although it may make sense to include subtraction and negation in the base language we have introduced, some may disagree with this design choice. This is because subtraction and negation can be emulated in the base language by multiplying by `-1`. However, introducing essential features as core language features can often improve the developer experience of programming in the language.

A more practical example of this contention is the introduction of CGP as a _native language feature_ into Rust. Technically speaking, we have gotten CGP working without extending Rust itself, and thus it may not be considered as an "essential" language feature.

However, if CGP gains widespread adoption, there are many benefits of having it as a native language feature in Rust, such as improved developer experience. But if Rust itself were already implemented with CGP, it would also be much easier to introduce an extended language with native CGP support, without needing to fork the Rust compiler itself.

### `MathPlusExpr` Expression Type

To demonstrate how CGP can help in such language extension, we will define a new `MathPlusExpr` expression type that can live side by side with the original `MathExpr` type:
...
The `MathPlusExpr` enum is similar to the `MathExpr` enum, but with additional `Negate` and `Minus` variants. For the `Plus`, `Times`, and `Literal` variants, we reuse the same sub-expression types that we have defined for `MathExpr`, but with the generic parameter changed to `MathPlusExpr`.

To support subtraction and negation, we also changed the value type for `Literal` from `u64` to `i64`. This would still work with the original providers like `EvalAdd` and `EvalMultiply`, as `i64` also implements the `Add` and `Mul` traits. This also demonstrates how generic implementations of `EvalAdd` and `EvalMultiply` and make them more reusable for different use cases as the language evolves.

For the `Negate` and `Minus` variants, we introduce new sub-expression types in the same way as before:
...
### Eval Provider Implementations

We would then implement the new evaluation providers for `MathPlusExpr`, starting with `Subtract`:
...
The subtraction evaluation is similar to addition and multiplication, with the only difference being that `Minus` is used as the input, and the `Sub` trait is used to perform the subtraction.

Similarly, we would implement the evaluation for `Negate` as follows:
...
Since `Negate` is a unary operator, we simply evaluate the inner expression, and then use `Neg` to negate the result.

### Wiring of `InterpreterPlus`

Finally, we would define a new `InterpreterPlus` context as follows:
...
#[cgp_new_provider]
impl ComputerRef<InterpreterPlus, Eval, MathPlusExpr> for DispatchExpr {
    type Output = i64;
...
Thanks to the modularity provided by CGP, we are able to implement `InterpreterPlus` with only few dozens lines of code. The main thing that is needed here is to dispatch the sub-expression types to the appropriate providers, and define a context-specific wrapper implementation to support recursive calls to `MatchWithValueHandlersRef`.

### Omitting To-Lisp Implementations

At this point, you might expect that we need to continue our implementation to also support the to-Lisp conversion for `MathPlusExpr`. However, when rapid prototyping on new language extensions, we often want to _skip_ implementing less essential features, such as to-Lisp conversion, and focus only on the core logic, such as evaluation.

The beauty of using CGP is that we can **choose not to** implement a language feature like to-Lisp conversion for `MathPlusExpr`, yet the standalone implementation of eval can **compile just fine** with no further action needed.

This is in stark contrast to typical Rust application designs that use **heavyweight traits** that include dozens if not hundreds of methods, which can be cumbersome to implement and extend. Typically, even if something like `MathPlusExpr` can be introduced into a Rust application, one would still need to fill in placeholder `todo!()` or `unimplemented!()` boilerplates on method implementations for functionalities such as to-Lisp conversion.

This often makes it challenging to determine which method must be implemented for an initial prototype, and which ones can be deferred to later phase, as one could only find out if an unimplemented method triggered a panic at runtime. In contrast, the minimal trait design and lazy wiring provided by CGP only checks for the implementation of a component only when it is actually needed.

Thanks to the flexibility and guarantees by CGP, once the code compiles, we are able to safely defer the implementation of to-Lisp conversion for `Minus` and `Negate` until later time, and not having to worry about its absence subtly causing the evaluation to crash at runtime.
