...
In the **Extensible Records** section, we have seen how the modular builder pattern allows us to split up the constructors of database client, HTTP client, and AI agents into independent builder providers. In the **Extensible Variants** section, we have also explored how the modular visitor pattern enables us to define the evaluation and to-Lisp conversion of each variant in a language expression enum into independent visitor providers.

Hopefully by now, you are convinced that these examples are practical enough to justify the use of CGP in real world applications. Nevertheless, in case if you still find the examples lacking real world use cases, you can probably skip reading the rest of this blog post and revisit CGP at a later time. This is because in the remaining sections are intended for audiences who want to dive deeper into the underlying implementation of extensible data types in CGP, so that they can **help** the CGP project to build the actual real world examples that you are looking for.

## Related Work

The implementation of extensible data types is inspired by many programming language research on similar topics. In particular, the CGP implementation is heavily based on the implementation of [datatype-generic programming in Haskell](https://wiki.haskell.org/index.php?title=Generics), and the paper [**Abstracting extensible data types: or, rows by any other name**](https://dl.acm.org/doi/10.1145/3290325). Similar features have also been implemented in other languages under different names, such as **row polymorphism** and **polymorphic variants**. Readers are highly encouraged to read the related work to get a better understanding of the underlying concepts used by CGP.

It may be too academic and too boring to explore all theoretical differences between CGP and others' approaches, and I am not obligated to include such section as I am not writing a research paper. But perhaps the most crucial differences of CGP's approach from others is the **seamless integration** of extensible data types with CGP itself.

## Row Constraint Propagation

Ultimately, the solution offered by CGP is to solve the following problem: given two generic functions that contain different row-like constraints, how can we **compose** them together to form a new generic function that can also compose and propagate the **constraints**?

To demonstrate this, we would use a simple example as follows:
...
Above we show two example functions that read the `first_name` and `last_name` fields from a generic `Context` type, and then format and return them as strings. We use the `HasField` trait to emulate a **row type** with read access to the field name, which is given with `symbol!` as a type-level string.

Suppose that we want to concatenate the output of both functions with a space in between, we would have to explicitly write a function like follows:
...
Here, the two `HasField` constraints must be explicitly specified in the function signature of `full_name_to_string` for the code to compile. This makes it challenging to implement such a function, especially when the implementation involves more constraints.

More importantly, it becomes challenging to write **generic** functions that can concatenate the output from *any* two functions with constraints, and return a new composed function that propagates the constraints. For example, consider the following naive attempt:
...
Our naive `concat_outputs` function is a *higher order function* that takes two closures `fn_a` and `fn_b`, and returns a new closure that calls both closures and concatenates their outputs with a space in between. Crucially, the constraints of the input closures are *not* specified in the signature of `concate_outputs`.

While the approach appear to work, we will quickly find out that we still need to manually propagate all the constraints when we declare the composed function, such as:
...
This is because we cannot declare simply declare `full_name_to_string` as a composition of `first_name_to_string` and `last_name_to_string` outside of a function body. So an expression like `let full_name_to_string = concate_outputs(first_name_to_string, last_name_to_string)` cannot itself be a top-level function that is exported by a module.

Traditionally, the approach to solve this problem is to introduce advanced language features such as **constraint kinds** or **row kinds** in order to make it possible to support such composition. As a result, this restriction has been a major reason why extensible data types have not been widely adopted in mainstream languages like Rust, which lack such features.

However, the main contribution with CGP is that we are now able to perform such composition today in Rust **without** needing more advanced language features. The key insight is to turn **functions** into **types**, and then use the **traits** and **type-level programming** techniques introduced by CGP to drive such composition:
...
The above code shows how we turn the `first_name_to_string` function into a `FirstNameToString` struct that implements `ComputerRef`, and similarly for `last_name_to_string`. We ignore the `Code` and `Input` parameters to simplify our example.

With the functions turned into types, we can now compose them by defining a `ConcatOutputs` provider as follows:
...
`ConcatOutputs` is implemented as a **higher order provider** that composes two providers, similar to how `concat_outputs` is a higher order function that composes two functions. It accepts two providers `ProviderA` and `ProviderB`, with the constraints that they implement `ComputerRef` with the output type implementing `Display`.

The main advantage of defining our functions as types that implement `ComputerRef`, is that we can now compose the two functions with just a type definition:

```rust
pub type FullNameToString = ConcatOutputs<FirstNameToString, LastNameToString>;
```

The crucial thing to notice here is that when defining `FullNameToString`, we do not need to know which constraints are hidden within `FirstNameToString` and `LastNameToString`. Instead, the constraints are **implicitly propagated** and **lazily resolved** only when the `FullNameToString` provider is used.

We can think of this programming technique as a form of **lazily type-level programming**, because the computation is done at the type-level, with the evaluation only happening **lazily** when the trait implementation is **used**.

This is the core programming technique that is used throughout CGP, and it is what enables us to use extensible data types without needing to wait for Rust to implement them natively.
