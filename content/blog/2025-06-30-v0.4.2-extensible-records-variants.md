...
We can now implement a provider for `ComputerRef` that transforms a `Plus` expression into a Lisp expression. The implementation is pretty straightforward:
...
We implement `PlusToLisp` as a provider for `ComputerRef` with `Plus<Expr>` as the input type. It works with any `Context` and `Code`, given that the context implements `HasLispExprType` and `CanComputeRef<Code, Expr>`, with `Context::LispExpr` as the output type.

The `HasLispExprType` trait is simply defined as a CGP abstract type as follows:
...
During the wiring steps later, we will instantiate the type to the concrete `LispExpr` type that we have defined earlier. However, since we want to decouple the concrete `LispExpr` type from the provider implementation, we would instead work with the *abstract* `LispExpr` type inside `PlusToLisp`.

### Sub-Expression Upcasting

Since we still need to construct our result with the abstract `LispExpr` type as the output, we instead introduce a *local* `LispSubExpr` type as follows:
...
Compared to the main `LispExpr` type, the `LispSubExpr` enum contains only a *subset* of variants that we need for our `PlusToLisp` implementation. That is, we only include the `List` and `Ident` variants, while excluding `Literal` or any additional variants that will be introduced to `LispExpr` in the future. Notice that the enum is still parameterized by the main `LispExpr` type, as the inner expressions still need to support all possible variants of `LispExpr`.

Inside the `where` clause, we then specify that `LispSubExpr<LispExpr>` needs to implement `CanUpcast<LispExpr>`, so that we can reuse the [**upcasting** mechanism](#safe-enum-upcasting) that we have introduced earlier to upcast `LispSubExpr` to `LispExpr`.

Inside the `compute_ref` method, we first transform the inner expressions from `Plus` into `LispExpr`. We then construct a Lisp identifier `+` by first constructing a `LispSubExpr::Ident` variant, and then upcasting it to `LispExpr` using the `upcast` method. Finally, we place all three `LispExpr` values into a `LispSubExpr::List` variant, and then upcast it to `LispExpr` using `upcast`.

Through this example, we demonstrates how the upcasting mechanism helps us to decouple the *construction* of an enum variant from the concrete enum type. With this, we will be able to reuse `PlusToLisp` for not just our own `LispExpr` type, but any future `LispExpr` type that supports the `List` and `Ident` variants.

Essentially, the upcasting mechanism through `#[derive(HasFields, ExtractField, FromVariant)]` offers the same flexibility for *enums* as the `#[cgp_auto_getter]` macro for *structs*. The main difference is that while CGP auto getter traits allows *reading* of subset of fields from a struct, the upcasting mechanism allows *construction* of enum from a subset of variants.
...
With the to-Lisp providers implemented, we can now update the wirings of `InterpreterComponents` to include them in the wiring of `ComputerRefComponent`:
...
We first use `UseType` to instantiate the abstract `LispExpr` type to the concrete `LispExpr` type that we have defined earlier. Following that, the wirings for `ComputerComponent` is the same as before for the eval implementation. We then add a new `UseInputDelegate` mapping for `ComputerRefComponent`, by mapping `Literal`, `Plus`, and `Times` to their respective to-Lisp implementations.

For the main `Expr` type, we dispatch it to `DispatchToLisp`, which is defined as a wrapper similar to `DispatchEval` as follows:
...
Instead of using `MatchWithValueHandlers`, we use a separate dispatcher provided by CGP, `MatchWithValueHandlersRef`, to dispatch reference-based inputs to providers implementing `ComputerRef`.

Behind the scene, `MatchWithValueHandlersRef` shares some common parts with `MatchWithValueHandlers`, but it also contains some specialized implementations, such as calling `ComputerRef` instead of `Computer`, and extracting each variant in the enum with borrowed values.

Thanks to Rust's type system, we can confidently implement different dispatchers such as `MatchWithValueHandlers` and `MatchWithValueHandlersRef`, all while ensuring that our implementations are provable correct. On the other hand, if the dispatchers were implemented with macros, it would be much easier to make simple mistakes such as having incorrect lifetime assumptions or using borrowed values when owned values are expected, with the mistakes not being caught during the implementation of the macros.

Aside from `MatchWithValueHandlersRef` and `MatchWithValueHandlersRef`, CGP also provides many other dispatchers that share the same underlying generic implementation primitives. In more advanced use cases, you could even implement your own custom dispatchers in the same way. As we can see, the generic-based dispatcher implementation offers more flexibility, extensibility, and interoperability with other parts of the codebase.
