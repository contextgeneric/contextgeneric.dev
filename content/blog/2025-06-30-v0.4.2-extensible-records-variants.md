...
As we can see from the previous example, in order to implement something like `BuildAndMergeOutputs`, all we have to do is to take a list of providers, wrap them around `BuildAndMerge`, and then compose them with `BuildWithHandlers`.

The wrapping pattern is common enough that CGP provides a `MapFields` trait to simplify the implementation:
...
Essentially, `MapFields` implements the type-level equivalent of `list.iter().map()`, with the `Mapper` type expected to implement `MapType`. This is the same `MapType` that we have used earlier to implement `IsPresent` and `IsNothing`.

### `BuildAndMergeOutputs`

With all the supporting constructs in place, we can finally implement `BuildAndMergeOutputs` as a composite of `BuildWithHandlers` and `BuildAndMerge`. First, we define a type mapper `ToBuildAndMergeHandler` to map a `Handler` type to `BuildAndMerge<Handler>`:
...
After that, we simply define `BuildAndMergeOutputs` as a *type alias* to `BuildWithHandlers`, with the `Handlers` type mapped through `ToBuildAndMergeHandler`:
...
This demonstrates the power of using type-level programming to compose the underlying implementation of `BuildAndMergeOutputs` into smaller and composable pieces. With the modular decomposition, we are able to easily build other forms of builder dispatchers, such as by building everything and setting directly to fields using `BuildAndSetField`.

Compared to macro-based approaches, it would be much more difficult to decompose a functionality like `BuildAndMergeOutputs` into smaller macros. This is because it is much more difficult to verify that the composition of two smaller macros is correct. On the other hand, the composition of constructs such as `BuildWithHandlers` still requires precise `where` constraints to be specified, to ensure that the composition itself is sound.
