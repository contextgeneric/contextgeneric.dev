...
To understand how `BuildAndMergeOutputs` works, we will try and manually implement a provider that does the same thing:
...
The above code shows the boilerplate provider implementation that we would have to write if we didn't use `BuildAndMergeOutputs`. First, we define the `BuildApp` as a _context-specific_ provider for `FullAppBuilder`, that implements `Handler` and works for any `Code` and `Input`.

Inside the body for `handle`, we build `App` in similar ways as we build `FooBarBaz` in the [earlier section](#safe-struct-building). We first initialize an empty `App` builder with `App::builder()`. We then call `BuildSqliteClient::handle` to build the SQLite client fields, and then call `build_from` to merge the results into the `App` builder. We repeat this process for the HTTP client and OpenAI client. Finally, we call `finalize_build` to construct the final `App` instance.

In the example `BuildApp`, we discard the original input, and pass in `()` to each of the sub-handler. In the actual implementation of `BuildAndMergeOutputs`, a reference to the intermediary builder is passed as the input instead, but we omit this detail for the sake of simplicity.

As we can see, while the manual `BuildApp` implementation is quite verbose, it is still pretty straightforward to understand what is going on inside. The use of `BuildAndMergeOutputs` helps reduce the boilerplate required to combine multiple builder handlers and threading the intermediary builders through each of them. Aside from that, operations are essentially as simple as the manual implementation.
