...
This macro allows us to delegate the implementation of various components of the builder context. First, we configure error handling by using the `cgp-anyhow-error` library. The `UseAnyhowError` component specifies that our unified error type will be `anyhow::Error`, and the `RaiseAnyhowError` component allows conversion from standard error types like `sqlx::Error` and `reqwest::Error` into this unified type.

For the `HandlerComponent`, we use a builder dispatcher called `BuildAndMergeOutputs`. This dispatcher enables us to construct the full `App` type by combining multiple builder providers. The `App` type is specified as the target of the build, and we pass a type-level list of the builder providers to be used, wrapped in `Product!`. In this example, we include `BuildSqliteClient`, `BuildHttpClient`, and `BuildOpenAiClient`, which we implemented earlier.
...
In this example, we initialize `FullAppBuilder` by filling in the required configuration values. We then call `builder.handle()` to construct the `App`. The `handle` method requires two arguments: a `Code` type and an `Input` value. However, because neither of these are used in our example, we simply pass the unit type `()` for both. This simplifies to the equivalent of calling `builder.handle()` in practice.
