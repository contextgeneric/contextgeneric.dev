...
For the top-level `Expr` type, we introduce `DispatchToLisp`, a dedicated dispatcher that routes the various expression variants to their corresponding transformation providers. It is defined as follows:
...
With both evaluation and Lisp transformation now wired into the same interpreter context, the system is able to evaluate expressions to numeric results or convert them into Lisp-style syntax trees, all from the same `Expr` type. The modularity, reusability, and compile-time guarantees of this architecture make CGP a powerful and scalable tool for building language runtimes and transformation pipelines in Rust.

## Advanced Techniques

### Binary Operator Provider

When examining the implementations of `PlusToLisp` and `TimesToLisp`, it quickly becomes clear that they follow nearly identical patterns. Aside from the specific operator symbol and the input types, the transformation logic is the same. This duplication presents a perfect opportunity for abstraction.
...
In the earlier sections, we have learned about the differences between the `Computer` and `ComputerRef` traits, and understand the value of using `ComputerRef` to allow the input to be borrowed instead of moved. When we revisit the implementation of eval implementations, it would become clear that we are better off using `ComputerRef`, as the evaluator don't really need to own the input value.

However, suppose that we have re-implemented `EvalAdd`, `EvalMultiply`, and others using `ComputerRef`, there is now an issue with the wiring: that we have previously already wired the implementation of `ComputerRefComponent` to the to-Lisp providers. In order to support both evaluation and to-Lisp conversion through `ComputerRef` we need some way to *dispatch* the computation based on the *intention*.

This is where the `Code` parameter in the `ComputerRef` trait comes into play. [Previously in Hypershell](/blog/hypershell-release/#generic-dispatcher), we have introduced the `Code` parameter and use it to define type-level DSLs for writing Hypershell programs. But the same `Code` parameter can also be used for much simpler use cases, such as to define "statements" about what kind of computation we want to perform.

Following the footsteps of Hypershell, we would first define two new DSL *syntaxes* to be used with our interpreter:
...
Compared to Hypershell, our interpreter DSL only needs two kinds of statements to distinguish the two computation operations: `Eval` for program evaluation and `ToLisp` for to-Lisp conversion.

With the syntax defined, we can now define a combined handler for `Plus`, which would perform computation with the `Plus` input based on the specified `Code`:
...
We use `delegate_components!` to define a new `HandlerPlus` provider struct, which contains a delegate mapping for `ComputerRefComponent`. We then specify the delegated value to be `UseDelegate`, which would perform generic dispatch based on the `Code` parameter to the type-level table defined in the new `PlusHandlers` struct. Inside this nested mapping, we map the handling of `Eval` to `EvalAdd` and `ToLisp` to `BinaryOpToLisp<symbol!("+")>`.

Through the blanket implementation of `ComputerRef` generated by CGP, this turns `HandlerPlus` into a provider for `ComputerRef`, which implements the trait by delegating the implementation to the actual providers `EvalAdd` and `BinaryOpToLisp`. This shows the usefulness of `delegate_components!` not just in top-level wiring of the context provider, but also for defining new providers as a *composition* of other providers.

Technically speaking, we could also define `HandlePlus` as a provider that implements `ComputerRef` for *both* `Eval` and `ToLisp` as separate impl blocks, such as:
...
#[cgp_provider]
impl<Context, Code, MathExpr> ComputerRef<Context, ToLisp, Plus<MathExpr>> for HandlePlus
{ ... }
```

That is, there are two `impl` blocks for `ComputerRef` for `HandlePlus`, but with the generic `Code` parameter replaced with the concrete types `Eval` and `ToLisp`.

While this approach also works, there are restrictions on where implementations of `Eval` and `ToLisp` can be defined, such as in the same crate. This reduces the flexibility of separating the code based on *functionality*, such as by grouping all `Eval` implementations in the same crate.

By using `delegate_components!`, we can define each part of `HandlePlus` totally separately, and then compose them only when we define the concrete context. This gives us much more flexibility on how to organize the code.

Similar to `HandlePlus`, we can similarly define the composed providers for `HandleTimes` and `HandleLiteral` as follows:
...
Finally, we combine the two dispatchers for `Eval` and `ToLisp` into a single dispatcher `HandleMathExpr`:
...
Once all the composed providers are defined, we can now setup the new wiring for `Interpreter` as follows:
...
Inside the wiring for `ComputerRefComponent`, we first use `UseInputDelegate` to dispatch the computer provider based on the input type. We then map each sub-expression type to the corresponding composed providers that we have defined earlier.

With this setup, we are effectively performing a *two-level* dispatch: first based on the `Input` type, and then based on the `Code` type. Naturally, it is also possible to define the dispatchers the other way around, starting with the `Code` type and then dispatching based on the `Input` type.

Ultimately, the dispatch ordering is determined by the specific use case or code organization of the project. For example, by grouping the `Code`-based dispatchers first, we can define providers like `HandlePlus` and `HandleTimes` in separate crates. This would imply that the project is better organized based on a specific sub-expression, rather than a specific operation.

Regardless of which dispatch ordering is chosen, it is worth keeping in mind that the choice of ordering does **not** affect the overall performance of the system. Because the dispatch is done at compile time, both ordering would result in the same generic instantiation, and thus incurs no runtime overhead.
