...
While the manual implementation of `BuildApp` is relatively easy to follow, it is also quite repetitive. The main benefit of `BuildAndMergeOutputs` is that it eliminates this boilerplate by abstracting away the repetitive logic of chaining multiple builder steps and threading intermediary results. Aside from this reduction in verbosity, the behavior remains conceptually the same as what is shown in the manual example.
...
To work around this, `serde` includes a broad set of 26 visitor methods in its core `Visitor` trait to support a wide range of cases. However, this introduces the opposite problem: when a serialization format **does not** support a specific visitor method, the only option is to return a runtime error. There is no way to signal at compile time that a type is incompatible with the format, even if it formally implements `Serialize` or `Deserialize`.
...
The example of `serde` illustrates how the traditional visitor pattern can create an awkward tension between being too **restrictive** (due to a closed interface) and too **permissive** (by accepting types that may not be fully supported). What we need is a visitor pattern that allows both sides—the data provider and the data consumer—to specify **precisely** what they support and require.

This is the core motivation behind the **extensible visitor pattern** in CGP. It provides a way to define visitors and handlers that are open for extension, while still preserving **compile-time safety**. We will explore in the next sections how this is done.

## The Expression Problem

Although it is possible to replace the visitor pattern used in `serde` with CGP, doing so would require non-trivial amount of work, and it would be too much to explain in this blog post. Instead, we will make use of an equivalent, but simpler and well-known problem as an example to demonstrate the capabilities of extensible visitors in CGP - the [**expression problem**](https://en.wikipedia.org/wiki/Expression_problem).

Suppose that we want to implement an interpreter for a toy programming language in Rust, and let's stay our language would contain just simple arithmetic expressions such as `1 + (2 * 3)`. Traditionally, to implement this toy language, we would define the _abstract syntax_ of the language as an enum such as:
...
That is, we define that an expression can be either in the form `a + b` for the `Plus` case, or `a * b` for the `Times` case, where `a` and `b` can be any sub-expression. The final case `Literal` indicates that the expression can also be a `u64` integer value. We wrap the inner `Expr` values inside `Box`, as the size requirements in Rust requires us to allocate field values with potentially unbounded size on the stack.

In order to evaluate our toy language, we would implement an `eval` function for our `Expr` program, such as:
...
The `eval` implementation of our toy language is very straightforward, as we just use Rust's native `+` and `*` operatos to handle the `Plus` and `Times` case. However, keep in mind that in the implementation of real world programming languages, each evaluation case may span dozens or hundreds of lines of code. Furthermore, the `Expr` enum may contain a lot more variants than covered in this toy example.

If you are looking for a more complex example of `Expr`, please refer to the [`Expr` enum in `syn`](https://docs.rs/syn/latest/syn/enum.Expr.html), which contains over 40 variants. For the purpose of making the example comprehensible to readers of all background, we will pretend that our example `Expr` is as complicated as the `Expr` type in `syn`.

A key challenge with the expression problem is that we would not be implementing just an `eval` function for `Expr`, but also many similar functions. For example, we might write a naive `expr_to_string` function that converts the expression to string, such as follows:
...
As the programming language grows in complexity, there will inevitably be needs to _extend_ the `Expr` enum with new variants, or _remove_ old variants from it. However, any modification to `Expr` will require updating the `match` expressions in _all_ functions like `eval` and `expr_to_string` to handle the updated variants. As a result, the `Expr` enum becomes **tightly coupled** with all functions that uses the type.

Traditionally, there is no simple ways to **decouple** functions like `eval` from enums like `Expr`. This is partly due to the recursive nature of the data structure - A sub-expression like `Plus` contains `Expr` itself as its child. Because of that, even if we add a sub-function like `eval_plus` to handle the sub-expression `Plus`, it would still often be tightly coupled with the main `eval` function to handle the inner expressions.

It is worth noting that this recursive structure is commonly found not just in programming language implementations, but also other forms of data structures. For example, a JSON `Value` may contain an inner `Map`, which in turns contains `Value` for each of the map values.

Due to this tight-coupling, it becomes difficult to _experiment_ or _extend_ the language, unless changes are made directly on the main `Expr` type. In some cases, this can be done in the form of a *merge request* to an open source project. However, updating the variants may require substantial reasoning due to the significant maintenence problems that it introduces. If an upstream project refuses to accept such changes, a downstream project may have no choice but to **fork** the entire project just so that it can use a version of `Expr` that contains the variants that it needs.

Next, we will take a look at how to use CGP to decouple the implementation of each variant from the concrete `Expr` enum.