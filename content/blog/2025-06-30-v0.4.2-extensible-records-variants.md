...
We will now look at how to implement a sub-part of `eval` with CGP to handle the `Plus` operator. In order to decouple from the concrete `Expr` type, we would first define a `Plus` sub-expression type as follows:

```rust
pub struct Plus<Expr>(pub Box<Expr>, pub Box<Expr>);
```

Rather than being part of the concrete `Expr` enum, we define `Plus` as a *struct* that is parameterized by a *generic* `Expr`, representing the root expression type. Inside the struct, we define two `Box<Expr>` fields to represent the left and right sub-expressions.

To implement the eval provider, we also make use of a **Computer** trait provided by CGP, which is defined as follows:
...
The `Computer` trait is similar to the `Handler` trait that we have covered earlier, with the main difference being that it has a *non-async* method and does not return any error in its result. It is given the name "computer", because it represents a *pure function* that performs *computation* on the given input, and returns the result as output.

similar to `Handler`, we make use of the `Computer` trait to implement our evaluator, because it is a *generalized trait* that can later be used to dispatch the computation of variants in a concrete enum.

With `Plus` and `Computer` defined, we now define a new `EvalAdd` provider for `Plus` as follows:
...
The `EvalAdd` provider implements the `Computer` trait with `Plus<Expr>` as the input. It works with any `Code` and `Context` type, with the constraint that `Context` implements `CanCompute<Code, Expr>`. In other words, we are making use of dependency injection to require the context to provide ways to evaluate the `Expr` type.

We also require the output returned from `CanCompute<Code, Expr>` to implement the `Add` trait, so that we can perform addition on it. With this, our plus evaluator can work not just with a specific integer type like `u64`, but also *any* type that implements `Add`, such as `u32` and `i64`.

Inside the method body, we simply call `compute` to evaluate the inner expressions, perform addition, and return the result as the output.

Similar to `Plus`, we also implement the eval provider for `Times` as follows:
...
And as for the `Literal` expression, we just return the literal value as the output:
...
As we can see in the example implementations, the providers `EvalAdd`, `EvalMultiply`, and `EvalLiteral` are fully *decoupled* from each other as well as the concrete expression types. In other words, each of the provider can live in completely separate crates or modules, and be combined to become a full evaluator later on.

## Evaluating Concrete Expressions

Now that we have defined the eval providers, let's define a concrete `Expr` type and implement eval using the providers. We would define `Expr` as follows:
...
Instead of inlining the sub-expression values in each variant, we define `Expr` to contain variants that contain the sub-expression structs that we have defind earlier. We have also recursively instantiated the generic type in `Plus` and `Times` with the `Expr` enum that we have just defined. For `Literal`, we choose `u64` as the primitive value.

We then define an empty `Interpreter` CGP context as follows:
...
For the example interpreter that we are implementing, there is no additional field that needs to be provided by the context, other than the compile-time wiring that we will add as follows:
...
Inside the wiring for `InterpreterComponents`, we set the provider for `ComputerComponent` to be a [**generic dispatcher**](/blog/hypershell-release/#generic-dispatcher) called `UseInputDelegate`, which would statically dispatch the implementation of `Computer` based on the generic `Input` type, by performing *type-level* lookup on the new `EvalComponents` table that we created.

The `new` keyword can be used in `delegate_components!` to automatically define a new struct, and in turns perform `delegate_components!` again on this newly created struct. Inside the inner mapping for `EvalComponents`, we map `Plus<Expr>` to `EvalAdd`, `Times<Expr>` to `EvalMultiply`, and `Literal<u64>` to `EvalLiteral`.

## Dispatching Eval

Additionally, we also delegate the main `Expr` type to `DispatchEval`, which is defined together with `Context` as follows:
...
We implement `DispatchEval` as a *context-specific* provider for `Interpreter`, with the concrete `Expr` type as the input. Inside the method body, we simply call a special **visitor dispatcher** called `MatchWithValueHandlers` to perform the dispatching of each variant in `Expr` to the respective providers that we have specified in `EvalComponents`.

The implementation of `DispatchEval` is essentially a **wrapper** around `MatchWithValueHandlers`. This is currently necessary, because Rust will otherwise return an "overflowing requirements" error when attempt to recursively implement the trait. This happens because in order to implement `Computer` for `MatchWithValueHandlers`, it needs to first implement `Computer` for variant handlers like `EvalAdd`, which in turns require `MatchWithValueHandlers` to implement `Computer`.

On the other hand, by defining a *context-specific* wrapper implementation, Rust would first "mark" the trait as being implemented, before continue to resolve the constraints inside of its method body. As a result, this hack helps us get around the cyclic dependency error and get the code compiled.

When calling `MatchWithValueHandlers`, the provider performs dispatching of each of the enum variant that is roughly equivalent to the following manual implementation:
...
Behind the scene, the underlying code is a bit more complicated than a simple `match` expression, as `MatchWithValueHandlers` is implemented *generically* without access to the concrete enum. However, it achieves the same thing so that we don't need to manually write the `match` expression ourselves, or generate the code from macros.

The number of lines of code saved in this example is not very significant, as it is not too tedious to manually write a `match` of 3 variants. But imagine if the same technique is applied to something like `syn::Expr` with over 40 variants, you may start to appreciate the amount of code saved by `MatchWithValueHandlers`.

Furthermore, CGP allows dispatchers like `MatchWithValueHandlers` to be implemented as generic implementations rather than macros. This makes the code easier to verify, and *provably* work with all enums that satisfy the constraints, independent of which enum it is instantiated with. As a result, the visitor dispatcher in CGP serves as a compelling alternative to other macro-based dispatchers currently available in the Rust ecosystem.