...
`Symbol!("url")` is the field name lifted into a type, which is what makes matching *by name* something
the trait system can do. Nothing here is inspected at run time; the list exists during compilation and
is gone afterwards. This is the same field-name-as-type idea behind an
[implicit argument](./implicit-arguments.md), scaled from one field to a whole struct.
...
What the machinery buys is the **extensible builder**: each subsystem is a provider producing its own
...
built from a different mix of the same providers is another table. What guarantees the mix is complete
is the same thing as above — if the providers between them do not supply every field, the build does not
...
another crate that has not is invisible to it — which is the honest difference between this and runtime
...
`timeout: u32` are producing the same field as far as merging is concerned. That is what makes the
decoupling work, and it means a rename in one place silently stops matching in another.
...
contributors must not know the whole — a plugin set, a framework building a user's type, several
