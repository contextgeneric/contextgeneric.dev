...
The family of computation components, and the three axes — synchronous or async, fallible or not,
taking an input or not — that tell them apart.
...
a future and carries `Async` in its name — `AsyncComputer` beside `Computer`.

**Infallible or fallible.** A fallible member returns a `Result` against the context's
[abstract error type](./modular-error-handling.md), and carries `Try` — `TryComputer` beside `Computer`.
...
The `Code` tag is what stops a context being limited to one computation per member of the family. Two
...
providers. Dispatch can key on the `Input` type as readily as on the `Code`, which is what
[dispatching](./dispatching.md) over extensible data uses, and what makes a program written as types
interpretable — see [type-level DSLs](./type-level-dsls.md).

## What it costs

**The `PhantomData` at every call site.** `app.compute(PhantomData::<()>, input)` is the price of the
tag being a type. For a context with one computation it is pure ceremony, and it is what most makes
handler code look unlike ordinary Rust.

**The family is a lot of names to hold.** Twelve or so components across three axes, plus the promotion
combinators, is more surface than any other part of CGP. The mitigation is the rule above — write the
