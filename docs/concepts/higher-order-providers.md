...
what you want is *the same transformation over any of them*.
...
value, nesting them costs nothing at runtime — the composition is resolved during compilation and what
executes is a direct call.
...
A provider trait carries the context as an explicit parameter — that is what the
[trait split](./consumer-and-provider-traits.md) does. So when a wrapper requires its inner provider to
implement the same capability, the requirement has to name that context slot:
...
What the attribute does *not* do is change the call. `Inner::area(self)` stays as it is — the inner
provider is invoked as an associated function with the context passed explicitly, because that is the
shape the provider trait actually has. It is the one place in a well-written provider where the trait
split is still visible, and trying to make it read as `self.area()` would be calling the context's own
wiring, which is a different thing entirely.
...
on its behalf, and none of the machinery on this page applies. What makes a provider higher-order is
that a parameter is *bound to a provider trait* and invoked to do part of the work.
...
**And the call form leaks.** `Inner::area(self)` is the one construct here that still reads
inside-out, and no attribute hides it. It is a small tax on every higher-order provider written.
