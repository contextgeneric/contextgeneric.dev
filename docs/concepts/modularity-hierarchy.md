...
What a real application lands on is separation for the axis where a wrong combination must be
**impossible** — a mock client must never reach production — and collapse for everything else. CGP decides
which variations are worth their own type; the tools you already use handle the ones that are not.
...
thing being operated on — `Rectangle: CanCalculateArea`. About the application means the wired type is one
you define to carry choices, often with no fields at all.
...
elaborate shape does not read as the intended destination. A capability about the application — send an
email, query a user, run the server — wired per application, with no type parameter anywhere, is the
ordinary case.
