...
Written by hand the pattern is repetitive. You state the supertraits twice, once on the trait and once on the
...
`foo_bar`. A caller of `foo_bar` sees a one-method interface and is shielded from the requirement
entirely.
...
The body appears twice, once as the trait's default and once in the impl, and the macro actually emits both
rather than simplifying. The impl supplies the method for every qualifying type;
...
the lifted parameter, which makes it a requirement on the *underlying* type rather than an assertion
about the alias.
...
such as a macro invocation in the body, is refused with *unsupported trait item*, because the macro has to
