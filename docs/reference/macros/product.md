...
That recursion is what makes structural, field-by-field operations possible at all. Because a struct's fields
...
[`#[derive(HasFields)]`](../derives/derive_has_fields.md); where you write `Product!` by hand is in a handler
...
Where you write `Product!` yourself is a handler pipeline, where the list *is* the program — the steps run
left to right:
...
- **Never hand-write the spine.** `Cons<A, Cons<B, Nil>>` is what `Product!` expands to, and writing it out is
...
This section shows the spine both macros build. You do not need it to use them, but the expanded form is what a
compiler error prints, so recognizing it is what makes those errors readable. `cargo cgp expand` resugars it
...
is exactly what `Product!` builds over the same elements' types. There is no magic layer between the two
