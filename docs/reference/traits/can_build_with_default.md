...
[upcast](./can_upcast.md)** — turning a `Point2d` into a `Point3d` whose extra `z` is `0`, naming no field
explicitly.

It chains [`builder()`](./has_builder.md), [`build_from`](./can_build_from.md), and
...
Two requirements come from the steps it chains. **The source needs [`HasFields`](./has_fields.md)**,
because [`build_from`](./can_build_from.md) walks the source's field list — the same requirement any merge
carries. And **every field the source does not supply needs `Default`**, because the finalize fills them.
...
- **[`CanBuildFrom`](./can_build_from.md) plus an explicit finalize** when some fields must be set by
...
- **[`CanUpcast`](./can_upcast.md)** for the enum analogue — widening a variant set rather than a field
  set.
...
Each bound is where one of the requirements comes from: [`CanBuildFrom`](./can_build_from.md) brings the
[`HasFields`](./has_fields.md) obligation on the source, and
[`CanFinalizeWithDefault`](./can_finalize_with_default.md) brings the `Default` obligation on the
remaining fields — through a [`TransformMapFields`](./transform_map_fields.md) walk carrying
...
[`CanBuildFrom`](./can_build_from.md) carries.
...
- [`CanBuildFrom`](./can_build_from.md) — the merge step, and where the source's requirements come from.
- [`CanFinalizeWithDefault`](./can_finalize_with_default.md) — the defaulting finalize step.
- [`HasBuilder`](./has_builder.md) — the builder it starts from.
- [`TransformMapDefault`](./transform_map_default.md) — the marker behind the defaulting.
- [`CanUpcast`](./can_upcast.md) — the enum analogue of widening.
- [`HasFields`](./has_fields.md) — what the source must derive.
