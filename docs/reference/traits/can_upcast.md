...
The trait itself is one method:
...
`Self` is the narrow enum, consumed; `Target` is the wider one. Both must derive the extensible-data
machinery, and the target's variant set must include every one of the source's — matched by name and by
payload type.
...
[`FromVariant`](./from_variant.md).

Because every source variant is guaranteed to exist in a wider target, the walk is total — and the
extractor left at the end is uninhabited, discharged with [`FinalizeExtract`](./finalize_extract.md).
...
variants there is no impl, and the failure is an unsatisfied bound rather than a runtime error — which is
...
- [`ExtractField`](./extract_field.md) and [`FromVariant`](./from_variant.md) — the two primitives the
