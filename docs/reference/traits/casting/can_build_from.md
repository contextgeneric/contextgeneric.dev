...
**The source needs [`HasFields`](../shape/has_fields.md) as well as a builder**, because the recursion walks the
...
[`TakeField`](../builder/take_field.md) to remove that value from the source and [`BuildField`](../builder/build_field.md)
to write it into the target builder, threading both the shrinking source and the growing builder through
the walk.

When the source's fields run out, the builder is returned, **not finalized**, which lets a second
`build_from` follow and is why `Output` is a builder type rather than the target struct.
