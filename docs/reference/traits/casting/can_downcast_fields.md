...
takes it by value, returning `Result<Target, Remainder>` just as `downcast` does. The returned
...
[`to_extractor`](../variant/has_extractor.md) on an enum to obtain the extractor, and this trait takes one it is
handed.
