...
`FieldGetter` is that shape. This is the ordinary consumer/provider duality applied to field access:
**you bound against [`HasField`](./has_field.md), and you wire `FieldGetter`**. They are not alternatives.

[`UseField`](../../providers/use_field.md) is the provider that implements it by reading a named field,
and [`#[cgp_getter]`](../../macros/cgp_getter.md) targets it.

## Definition

`FieldGetter<Context, Tag>` is the provider-trait mirror of [`HasField`](./has_field.md), with the context
...
`Self` is the **provider**, a zero-sized marker carrying no data, and `Context` is the type the field is
read from. `Tag` names the field, exactly as it does on `HasField`, and `Value` is the field's type.
`get_field` takes the context by shared reference and returns `&Self::Value`, with the `PhantomData<Tag>`
argument disambiguating which field is meant.
...
with the getter and tag `'static`, which [`ChainGetters`](../../providers/chain_getters.md) relies
on.
