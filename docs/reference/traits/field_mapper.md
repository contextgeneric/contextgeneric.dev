...
in provider-trait shape, with the context as an explicit type argument rather than as `&self`:
...
`Self` is the provider; `Context` is the type being read from. It is a supertrait extension of
[`FieldGetter`](./field_getter.md), exactly as [`MapField`](./map_field.md) extends
[`HasField`](./has_field.md) — so the group's two-by-two shape holds here too:
...
getter and the tag `'static`, so every field-getter provider gains `map_field` for free — which is
precisely what makes chaining composable without each provider opting in.
