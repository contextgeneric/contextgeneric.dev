...
What makes it the family's safety argument is not the method but **where the impl exists**. It is
...
- **Use [`CanFinalizeWithDefault`](./can_finalize_with_default.md)** when unset fields should be filled
...
- **Use [`FinalizeOptional`](./finalize_optional.md)** when absence should be *reported* at run time
...
[`CanFinalizeWithDefault`](./can_finalize_with_default.md) runs a
...
[`FinalizeOptional`](./finalize_optional.md), a different trait in a different crate.
...
**The enum side has its own ending.** [`FinalizeExtract`](./finalize_extract.md) discharges an exhausted
...
- [`CanFinalizeWithDefault`](./can_finalize_with_default.md) and
  [`FinalizeOptional`](./finalize_optional.md) — the two relaxed endings.
- [`MapType`](./map_type.md) — the `IsPresent` marker every field must reach.
- [`FinalizeExtract`](./finalize_extract.md) — the enum family's ending, sound for the opposite reason.
