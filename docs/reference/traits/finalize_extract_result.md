...
happen — and `FinalizeExtractResult` is what says so:
...
`Self` is the `Result`, and `Output` is the payload alone. It is implemented for any `Result<T, E>` whose
error half can be discharged:
...
tried, which is what makes it a proof rather than an assertion.
...
whose error is `Infallible` or [`Void`](../types/type_level_spines.md). That is occasionally useful
