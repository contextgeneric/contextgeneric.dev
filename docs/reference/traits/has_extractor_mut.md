...
`ExtractorMut<'a>` is a **generic associated type** with a `where Self: 'a` clause, and
`extractor_mut(&mut self)` returns one at the anonymous lifetime — so the call reads with no lifetime
written, and the borrow ends where the extractor is dropped.
...
per-variant [`MapType`](./map_type.md) markers continue to track possibility. That is the whole
...
generic over the [`MapTypeRef`](./map_type_ref.md) marker too.
...
- [`MapTypeRef`](./map_type_ref.md) — the `IsMut` marker this fixes.
- [`MapType`](./map_type.md) — the per-variant markers it composes with.
- [`HasFieldMut`](./has_field_mut.md) — the record side's mutable access.
