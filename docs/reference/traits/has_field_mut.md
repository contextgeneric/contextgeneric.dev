...
It is a **supertrait extension** rather than an alternative: `HasFieldMut<Tag>` requires
`HasField<Tag>`, and the field's type comes from that supertrait's `Value`. So bounding on the mutable
form gives you the read as well.
