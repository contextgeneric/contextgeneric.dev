...
`into_builder(self)` consumes an existing value and hands back a partial one with **every field
present**. That is what you want when *redistributing* a struct's fields rather than assembling one —
taking a value apart, moving some fields elsewhere, and rebuilding.
...
Unlike `builder()`, this one is a method taking `self`, so it is called on a value:
`person.into_builder()`. `Builder` names the same partial companion type
[`HasBuilder`](./has_builder.md) names, at the all-present configuration rather than the all-absent one.
...
- **Use [`ToFields`](./to_fields.md)** instead when what you want is the value's *shape* as a flat list
...
- [`TakeField`](./take_field.md) — removing a present field, which is what this enables.
...
- [`ToFields`](./to_fields.md) — the flat-shape alternative to a partial value.
- [`#[derive(BuildField)]`](../derives/derive_build_field.md) — generates this impl.
- [`MapType`](./map_type.md) — the markers the configuration is written in.
