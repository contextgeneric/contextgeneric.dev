...
default provider. `DefaultImpls1` adds a second type to the key, which a **per-type default**
...
which lets the projection `<Key as Trait<…, Delegate = Provider>>` resolve cleanly.
