...
`HasFieldsRef` carries one generic associated type and no method:
...
`Self` is the type being described. `FieldsRef<'a>` is the same shape as [`HasFields`](./has_fields.md)'s
`Fields`, with each value borrowed for `'a`. It is a **generic associated type**, which lets one trait
describe the shape at whatever lifetime a caller has, and the `where Self: 'a` clause keeps the borrowed
shape from outliving the value it describes. There is no method, because the trait only names a type; the
conversion that produces a value in this shape is [`ToFieldsRef`](./to_fields_ref.md).

The two shape traits are independent rather than one extending the other, and that is deliberate: a type
may be described owned, borrowed, or both.
