...
The value comes out owned; `Remainder` is the partial record with that one field's marker flipped back to
`IsNothing`, so it can no longer be finalized until the field is put back.
...
- **Use [`CanBuildFrom`](./can_build_from.md)** to move every shared field from one record into another's
...
- **Use [`ToFields`](./to_fields.md)** when the whole value should become a flat shape rather than a
...
The primitive's signature is what makes one trait serve both directions: `update_field` takes the new
storage in and hands the old storage back, so under this transition it takes `()` in and hands the real
value back.

**Its heaviest user is [`CanBuildFrom`](./can_build_from.md).** That recursion walks the source's field
...
- [`CanBuildFrom`](./can_build_from.md) — the merge that uses this internally.
...
- [`MapType`](./map_type.md) — the `IsPresent`/`IsNothing` markers this moves between.
- [`ToFields`](./to_fields.md) — the flat-shape alternative to decomposing a partial value.
