...
`TransformMap` carries that missing half — the actual function:
...
`Self` is a transform marker of your own naming; `M1` is the field's current state, `M2` the state it
should end in, and `T` the field's value type. So one impl answers one question: *given a field
currently in state `M1`, produce the same field in state `M2`.*
...
source marker's `Map<T>` — `T`, then `()`, then `Option<T>` — which is what makes the three impls
non-overlapping.
...
configuration [`FinalizeBuild`](./finalize_build.md) accepts.
...
- **Use the [optional-field layer](./can_finalize_with_default.md)** if what you want is defaulted or
  optional finalization. Both already exist, built on exactly this, and
  [`TransformMapDefault`](./transform_map_default.md) and
  [`TransformOptional`](./transform_optional.md) are the two markers it ships.
...
That is what lets three impls for one marker coexist without overlapping: they differ in `M1`.

[`TransformMapFields`](./transform_map_fields.md) is what calls this. For each field of the target's
[`HasFields`](./has_fields.md) shape, it uses [`UpdateField`](./update_field.md) to take the field out —
...
- [`MapType`](./map_type.md) — the markers naming each state, and the trait this supplies the missing
  half of.
- [`TransformMapDefault`](./transform_map_default.md) and [`TransformOptional`](./transform_optional.md)
  — the two markers CGP ships, and the models to copy.
- [`CanFinalizeWithDefault`](./can_finalize_with_default.md) — the capability built from the first of
  them.
- [`UpdateField`](./update_field.md) — the primitive the walk uses to take a field out and write it back.
