...
`MapType` is that marker's trait. It lets the [builder](../builder/has_builder.md) and
[extractor](../variant/extract_field.md) families move a value through intermediate states without
changing its runtime representation. A builder starts every field `IsNothing` and flips each to
`IsPresent` as it is filled; an extractor starts every variant `IsPresent` and flips each to `IsVoid`
as it is ruled out. Because a flip is a change of *type parameter*, the compiler tracks exactly which
fields are filled, and refuses to finalize an incomplete one.
...
state, expressed as a type. Because `Map<T>` is a **generic associated type**, a bound that pins the
storage reads `M: MapType<Map<String> = String>`, and one that leaves it open is plain `M: MapType`.
...
transition is a change of type argument. [`UpdateField`](../builder/update_field.md) performs one such
