...
itself, `IsNothing` stores `()`. So setting an absent field takes the real value in and hands `()` back,
and removing a present one takes `()` in and hands the real value back — one signature covering both
directions.

**This is what the derive actually writes.** [`BuildField`](./build_field.md) and
...
Reading the associated types is most of using it. `Value` is the field's declared type, stripped of any
marker; `Mapper` is the marker the field was in *before* the call, which the impl reports rather than
requires; and `Output` is the partial type with that one field now in `M`.

The `M` parameter is the target marker and is usually inferred from the value you pass — supplying a
...
- **`UpdateField`** for anything else — a transition to or from `IsOptional`, which is what the
  [optional-field extensions](./set_optional.md) do, or a transform's intermediate steps.
- **[`TransformMapFields`](./transform_map_fields.md)** when *every* field changes state rather than one.
...
`Mapper` being an *output* rather than an input is what lets the two directional traits pin a transition
...
- [`MapType`](./map_type.md) — the markers a field moves between.
- [`TransformMapFields`](./transform_map_fields.md) — the whole-record form, which calls this twice per
  field.
- [`SetOptional`](./set_optional.md) — a transition this covers and the directional traits do not.
