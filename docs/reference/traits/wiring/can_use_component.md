...
**You never name this trait.** It is the bound [`check_components!`](../../macros/check_components.md) asserts, and the
reason that macro produces a legible error instead of a cascade.

## Definition

`CanUseComponent` is an empty marker trait, and its entire meaning lives in a single blanket impl:
...
`Self` is the context being checked. `Component` is the component-name marker, the same key the wiring
table uses. `Params` collects the component's extra generic parameters and defaults to `()` when there
are none, filled by the same rule [`IsProviderFor`](./is_provider_for.md) uses: one parameter directly,
several as a tuple. The trait has no method, so the blanket impl carries the meaning. Its two bounds are
the two halves of "can use": the context must hold a [`DelegateComponent`](./delegate_component.md) entry
for the component, and that entry's provider must satisfy [`IsProviderFor`](./is_provider_for.md) for this
context and these parameters. Because `IsProviderFor` re-exposes the provider's real `where` bounds,
requiring it forces the compiler to evaluate and report them.

## Usage

The trait is in the prelude. You do not name it directly; you assert it through a check macro.
...
**Reach for [`check_components!`](../../macros/check_components.md), which reaches for this trait.** The one
