...
`DelegateComponent` carries a single associated type and no method, and it wears a diagnostic attribute
that shapes the error when a lookup misses:
...
`Self` is the table: the context or the provider bundle that owns the entry. `Key` is the type being
looked up, and it is `?Sized` so that an unsized marker can serve as one. `Delegate` is the value stored
at that key, the provider or the further table the entry resolves to. There is no method and no data; the
trait only associates one value type with one key type on a carrier type, so the whole lookup happens
while the compiler type-checks. The `#[diagnostic::on_unimplemented]` attribute reports a missed lookup as
a missing *entry* rather than an opaque "trait not implemented", one of the few diagnostics CGP improves
without a tool.
...
literally the projection, and the call it forwards to is resolved statically and inlined. The
[`#[diagnostic::on_unimplemented]`](#definition) message shown above turns an unwired component into a
readable missing-*entry* error rather than an opaque unimplemented trait.
