...
candidate impl, with no blanket competing with it, the compiler commits to it and reports precisely which
constraint failed.

**This is invisible plumbing until you read an error.** You never write it and never call it. It buys the
difference between "the trait is not implemented" and "the field `name` is missing on `App`", and it is
...
The four types together identify one provider-trait implementation. `Self` is the **provider** whose
validity is asserted. `Component` is the component-name marker, the same key the wiring table uses.
`Context` is the context the provider trait is implemented for. `Params` collects the component's own
extra generic parameters, and it defaults to the unit tuple `()` for a component that has none; how
several parameters are grouped is covered under [How `Params` is filled](#how-params-is-filled). The
...
The trait is in the prelude, so `use cgp::prelude::*;` names it. You never implement it; the useful
operations are supplying `Params` correctly and, occasionally, asserting the marker.
...
than by hand. That form is the tool for localizing a failure inside a stack of
...
[`delegate_components!`](../../macros/delegate_components.md) emits the forwarding. You instead read it in
an error and, occasionally, assert it through a check.
...
**The supertrait link.** [`#[cgp_component]`](../../macros/cgp_component.md) emits every provider trait
...
This binds a provider's dependency set to the marker: using the provider trait *requires*
establishing `IsProviderFor` first, so probing the marker is equivalent to probing the provider trait.

**The per-provider impl.** [`#[cgp_provider]`](../../macros/cgp_provider.md) and
...
Identical `where` clauses mean the marker is satisfiable exactly when the provider trait is. That is the
whole trick.

**The table forwarding.** [`delegate_components!`](../../macros/delegate_components.md) emits, for each
...
`GreetHello` requires. It also carries requirements *across layers*: when a provider bundle delegates
