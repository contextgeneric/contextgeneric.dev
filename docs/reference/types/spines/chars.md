...
One character of a type-level string: the spine that spells a field name out one character at a time so
...
time through a recursive `Chars` spine, terminated by [`Nil`](nil.md), the same way the product spine
...
- [`Cons`](cons.md) — the general product spine this one specializes, with a type head rather than a
  `const char`.
