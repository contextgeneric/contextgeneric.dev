...
The end marker of the product, string, and path spines: an empty, constructible list.

:::info

### Generated machinery

**You are not expected to write `Nil` by hand.** It is produced by the sugar that builds the spines it
...
The same marker terminates three of CGP's four spines. It ends the product spine built from
[`Cons`](cons.md), the string spine built from [`Chars`](chars.md), and the path spine built from
[`PathCons`](path_cons.md). Only the sum spine ends differently, in the uninhabited [`Void`](void.md),
...
`Nil` is the base case of every recursion over a spine it terminates. An operation that folds over a
product implements the step for [`Cons<Head, Tail>`](cons.md) and the base case for `Nil`, and the
recursion bottoms out when it reaches `Nil`. The string and path spines do the same: a walk over
...
`product![]` is `Nil` as a value. This is what a builder or a conversion returns when a shape has no
...
- **Expect `Nil`, not [`Void`](void.md), at the end of a record, a string, or a path.** A sum ends in
  `Void`; the other three spines end in `Nil`. Meeting the wrong terminator in an error usually means the
...
**`Nil` terminates three spines, not just the product one.** A `Nil` at the end of a
...
- [`Void`](void.md) — the sum spine's uninhabited end marker, the counterpart to this one.
- [`Chars`](chars.md) and [`PathCons`](path_cons.md) — the string and path spines `Nil` also terminates.
