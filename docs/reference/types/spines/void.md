...
The uninhabited end marker of the sum spine: an empty enum that can never be constructed, which is what
...
`Void` marks the end of a sum, and unlike the [`Nil`](nil.md) that ends the other spines, it is
...
for the sum spine is a type with no values at all.

That uninhabitedness is not a technicality; it makes the sum spine work. An [`Either`](either.md)
...
`Void` is the base case of a recursion over a sum, and it behaves differently from the product spine's
...
why the sum spine terminates in `Void` and the product spine in `Nil`.
...
- [`Nil`](nil.md) — the product, string, and path spines' constructible end marker, the counterpart to
  this one.
